...
const SESSION_KEY = "pttFieldLogger.sessions.v0.2.2";
const COUNTER_KEY = "pttFieldLogger.counter.v0.2.2";
...
async function startSession(startReason = "unknown") {
  ...
  try {
    recordedChunks = [];
    mediaRecorder = new MediaRecorder(micStream, { mimeType: getBestMimeType() });

    mediaRecorder.addEventListener("dataavailable", event => {
      if (event.data && event.data.size > 0) recordedChunks.push(event.data);
    });

    mediaRecorder.addEventListener("stop", () => finalizeRecording(currentSession?.id));
...
async function finalizeRecording(sessionId) {
  if (!sessionId || recordedChunks.length === 0) {
    setRecorderStatus("No audio captured for last session.");
    return;
  }
  ...
  try {
    ...
    logRaw(`Audio saved for ${sessionId}: ${formatBytes(audioBlob.size)}`);
  } catch (err) {
